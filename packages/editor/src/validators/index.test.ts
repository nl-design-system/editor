...
describe('runValidation adapter', () => {
  it('adapts detection results into a Range-keyed map', async () => {
    ...
    const entry = byKey(map, validations.HEADING_MUST_NOT_BE_EMPTY);
  ...
  it('attaches a correct action for correctable block issues', async () => {
    const { map } = await validate('<h1></h1>');
    expect(typeof byKey(map, validations.HEADING_MUST_NOT_BE_EMPTY)?.correct).toBe('function');
  });
  ...
  it('detects block, inline and document rules together', async () => {
    ...
    expect(byKey(map, validations.DOCUMENT_MUST_HAVE_CORRECT_HEADING_ORDER)).toBeDefined();
    expect(byKey(map, validations.INLINE_SHOULD_NOT_BE_UNDERLINED)).toBeDefined();
  ...
  it('reports every duplicate heading-one as its own entry', async () => {
    ...
    expect(allByKey(map, validations.DOCUMENT_MUST_HAVE_SINGLE_HEADING_ONE)).toHaveLength(2);
  });
...
describe('rule filtering through settings', () => {
  it('disableRules removes a rule from the results', async () => {
    const settings: EditorSettings = {
      disableRules: [validations.HEADING_MUST_NOT_BE_EMPTY],
      enableRules: ['*'],
      topHeadingLevel: 1,
    };
    const { map } = await validate('<h1></h1>', settings);
    expect(byKey(map, validations.HEADING_MUST_NOT_BE_EMPTY)).toBeUndefined();
  });
  ...
  it('enableRules limits detection to the named rules (kebab-case accepted)', async () => {
    ...
    expect(byKey(map, validations.HEADING_MUST_NOT_BE_EMPTY)).toBeDefined();
    expect(byKey(map, validations.INLINE_SHOULD_NOT_BE_UNDERLINED)).toBeUndefined();
  });
  ...
  it('honours a non-default topHeadingLevel', async () => {
    ...
    expect(byKey(map, validations.DOCUMENT_MUST_HAVE_TOP_LEVEL_HEADING_ONE)).toBeUndefined();
  });
});
...
describe('correct actions mutate the document', () => {
  ...
  it('unwraps an underlined mark', async () => {
    ...
    const entry = byKey(map, validations.INLINE_SHOULD_NOT_BE_UNDERLINED)!;
    entry.correct!();
  ...
  it('demotes a duplicate heading one to heading two', async () => {
    ...
    const entry = byKey(map, validations.DOCUMENT_MUST_HAVE_SINGLE_HEADING_ONE)!;
    entry.correct!();
  ...
  it('promotes a heading level to fix heading order', async () => {
    ...
    const entry = byKey(map, validations.DOCUMENT_MUST_HAVE_CORRECT_HEADING_ORDER)!;
    entry.correct!();
  ...
  it('opens the image dialog for a missing alt text', async () => {
    ...
    const entry = byKey(map, validations.IMAGE_MUST_HAVE_ALT_TEXT)!;
  ...
  it('converts a list-like paragraph into a semantic list', async () => {
    ...
    const entry = byKey(map, validations.PARAGRAPH_SHOULD_NOT_RESEMBLE_LIST)!;
    entry.correct!();
