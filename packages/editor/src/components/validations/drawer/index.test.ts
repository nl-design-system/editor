...
describe('<clippy-validations-dialog>', () => {
  ...
  it('renders large validations map with all validation items', async () => {
    ...
    const validationsMap: Map<Range, ValidationResult> = new Map([
      entry(elementValidations.HEADING_MUST_NOT_BE_EMPTY, 'error'),
      entry(elementValidations.IMAGE_MUST_HAVE_ALT_TEXT, 'error'),
      entry(inlineValidations.LINK_SHOULD_NOT_BE_TOO_GENERIC, 'warning'),
      entry(elementValidations.NODE_SHOULD_NOT_BE_EMPTY, 'warning', { nodeType: 'paragraph' }),
      entry(inlineValidations.MARK_SHOULD_NOT_BE_EMPTY, 'error', { nodeType: 'link' }),
      entry(documentValidations.DOCUMENT_MUST_HAVE_CORRECT_HEADING_ORDER, 'error', {
        headingLevel: 3,
        precedingHeadingLevel: 1,
      }),
      entry(elementValidations.PARAGRAPH_MUST_USE_SEMANTIC_LIST, 'warning', { prefix: '-' }),
      entry(elementValidations.HEADING_SHOULD_NOT_CONTAIN_BOLD_OR_ITALIC, 'warning'),
      entry(inlineValidations.MARK_SHOULD_NOT_BE_UNDERLINED, 'warning'),
