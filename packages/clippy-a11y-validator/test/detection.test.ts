...
describe('content detection', () => {
  it('flags an empty heading', () => {
    expect(keys('<h1></h1>')).toContain(headingValidations.HEADING_MUST_NOT_BE_EMPTY);
  });
  ...
  it('flags an image without alt text but not one with alt text', () => {
    expect(keys('<p>x</p><img src="a.png">')).toContain(imageValidations.IMAGE_MUST_HAVE_ALT_TEXT);
  ...
  it('flags list-like sibling paragraphs', () => {
    expect(keys('<p>- one</p><p>- two</p>')).toContain(paragraphValidations.PARAGRAPH_SHOULD_NOT_RESEMBLE_LIST);
  });
...
describe('inline content detection', () => {
  ...
  it('does not flag inline elements with text', () => {
    expect(keys('<p>text <b>bold</b></p>')).not.toContain(richTextContentValidations.INLINE_SHOULD_NOT_BE_EMPTY);
  });

  it('flags an underlined element', () => {
    expect(keys('<p>text <u>underlined</u></p>')).toContain(richTextContentValidations.INLINE_SHOULD_NOT_BE_UNDERLINED);
  });
...
describe('collectTreeValidations', () => {
  ...
  it('isolates a throwing tree validator so the others still produce results', () => {
    ...
    const ok: TreeValidator = (dom) => [{ element: dom, scope: 'block', severity: validationSeverity.INFO }];
    const spy = vi.spyOn(console, 'error').mockImplementation(() => {});
